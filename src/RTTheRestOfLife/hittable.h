...
class translate : public hittable {
    ...
    public:
        shared_ptr<hittable> ptr; //the original object
        vec3 offset;
};

bool translate::hit(const ray& r, double t_min, double t_max, hit_record& record) const {
    //Trick: Move the ray instead of the original object
    ray moved_r(r.origin() - offset, r.direction(), r.time());
    if (!ptr->hit(moved_r, t_min, t_max, record)) 
        return false;

    //Add the offset to get the real intersection point
    record.p += offset;
...
bool rotate_y::hit(const ray& r, double t_min, double t_max, hit_record& record) const {
    ...
    //Trick: rotate the ray -theta degrees
    ...
    //rotate p and normal theta degrees
